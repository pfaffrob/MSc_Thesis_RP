...
function AcronymsPandoc.replaceExistingAcronym(
    ...
    local style = style or acronym.style or Options["style"]
...
function AcronymsPandoc.generateDefinitionList(sorted_acronyms)
    ...
    for _, acronym in ipairs(sorted_acronyms) do
        ...
        -- One paragraph: longname + hard line break + definition (if any)
        local value_inlines = {}
        for _, x in ipairs(Helpers.ensure_inlines(acronym.longname)) do
            table.insert(value_inlines, x)
        end
        if acronym.definition and Helpers.inlines_to_string(acronym.definition) ~= "" then
            table.insert(value_inlines, pandoc.LineBreak()) -- plain line break
            for _, x in ipairs(Helpers.ensure_inlines(acronym.definition)) do
                table.insert(value_inlines, x)
            end
        end
        local definition_value = pandoc.Para(value_inlines)

        -- One block only (paragraph). The list itself provides the paragraph break after each item.
...
function AcronymsPandoc.generateCustomFormat(sorted_acronyms, loa_format)
    ...
    for _, acronym in ipairs(sorted_acronyms) do
        ...
        -- Optional: user can reference {definition} in template
        local serialized_definition = ""
        if acronym.definition and Helpers.inlines_to_string(acronym.definition) ~= "" then
            serialized_definition = Helpers.serialize_inlines(acronym.definition)
        end
        acronym_markup = acronym_markup:gsub("{definition}", serialized_definition)
