...
function replaceAcronym (args, kwargs, meta)
    -- Filter out empty or whitespace-only arguments
    local filtered_args = {}
    for _, arg in ipairs(args) do
        local arg_str = pandoc.utils.stringify(arg):match("^%s*(.-)%s*$")  -- trim whitespace
        if arg_str ~= "" then
...
function generateListOfAcronyms (args, kwargs, meta)
    ...
    if #filtered_args ~= 0 then
        quarto.log.warning(
            ...
            "expected 0, found", tostring(#filtered_args), "."
        )
    end
